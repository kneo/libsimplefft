				...
				for(butterfly=0;butterfly<butterflies;butterfly++){ //compute n butterflies
					...
					switch(fft_type){ // compute the type independent fft
						...
						case CPLX_TYPE_BYTE:
							...
							re_8t[pos_even] = (int8_t) roundf(re_mul_f((float)diff_re_8t, (float)diff_im_8t, twiddle_re_ft[pos_t], twiddle_im_ft[pos_t])); //butterfly wing 2
							im_8t[pos_even] = (int8_t) roundf(im_mul_f((float)diff_re_8t, (float)diff_im_8t, twiddle_re_ft[pos_t], twiddle_im_ft[pos_t]));
